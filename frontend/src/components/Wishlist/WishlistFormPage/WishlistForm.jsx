...
export default function WishlistForm(props) {
  ...
  function sendDetails() {
    if (id) {
      ...
      axios
        .post("http://localhost:8070/wishlist/", wishlist)
        .then(() => {
          window.location.reload();
        })
        .catch((err) => {
  ...
  function getCategory() {
    console.log(type);
    axios
      .get(`http://localhost:8070/product/anitem/${type}`)
      .then((products) => {
  ...
  return (
    <div className="wishlist-form">
      <h4>
        {" "}
        {id
          ? "Change " + wishlistDetails.wishlistName + " details"
          : "Try to make a list for future purchases."}
      </h4>

      <div className="wishlist-text">
        <TextField
          placeholder="Name"
          value={wishlist.wishlistName}
          name="wishlistName"
      ...
      <div className="wishlist-text">
        <TextField
          id="standard-basic"
          value={wishlist.wishlistDescription}
      ...
      <div className="wishlist-text" style={{ display: "flex" }}>
        <div style={{ flex: "15" }}>
          ...
          <Select
            ...
            style={{ textAlign: "left" }}
          >
            <MenuItem value={"Vegetables"}>Vehicle Oil</MenuItem>
            <MenuItem value={"Meat"}>Bicycle</MenuItem>
            <MenuItem value={"Seafood"}>Vehicle Body</MenuItem>
          </Select>
        </div>
        ...
        <div style={{ flex: "2" }}>
          ...
          >
            <UilSearch
              size="28"
              style={{ position: "relative", top: "20%", right: "3%" }}
            />
          </div>
        </div>
      </div>
      ...
      <div className="wishlist-button">
        <Button onClick={sendDetails} style={buttonStyle}>
          {id ? "Change" : "Create"}
        </Button>
      </div>
    </div>
